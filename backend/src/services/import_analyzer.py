...
class ImportAnalyzer:
    ...
    @classmethod
    async def infer_categories(cls, course_names: list[str]) -> dict[str, str]:
        """Use LLM to infer course categories from names when category column is missing."""
        from src.services.llm.factory import LLMFactory
        ...
        prompt = (
            "根据以下课程名称推断每门课程的分类（如：专业必修、通识选修、公共基础等）。\n"
            f"课程列表:\n{json.dumps(course_names, ensure_ascii=False)}\n\n"
            '返回 JSON: {"categories": [{"name": "课程名", "category": "推断分类"}]}'
        )
        ...
        try:
            ...
            return {item["name"]: item["category"] for item in result.get("categories", [])}
        except Exception as e:
